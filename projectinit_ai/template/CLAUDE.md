...
4. **Validation-Separation:** Las pruebas de robustez/falsificación deben ir en scripts separutilities en `/02_scripts/validation/`.
...
6. **Documentation Standard:** Todo script debe estar autofrenado con comentarios técnicos que expliquen la lógica de cada bloque. Sin emojis. Consistencia absoluta en el tono bilingüe (Python/Stata).
7. **Execution Proactivity:** Si el entorno tiene Stata/Python disponible, el agente ejecutará los scripts automáticamente para validar outputs. En Stata, utiliza siempre el modo batch: `stata-mp -b do file.do`.
...
- `05_doc/` (Manuscrito, presentaciones, projectinit_aer.sty)
- `06_admin/` (REPD.md, IRB, PAP Registrations, NDA)
...
| `/split-pdf` | Lee PDFs pesutilities bloque a bloque sin desbordar contexto. |
