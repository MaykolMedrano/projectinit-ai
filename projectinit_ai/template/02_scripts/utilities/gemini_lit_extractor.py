...
Toma PDFs masivos ubicutilities en `04_literature/papers/` y usa el inmenso contexto
...
Uso:
- python 02_scripts/utilities/gemini_lit_extractor.py [opcional: paper.pdf]
"""
...
try:
    ...
    print("Asegúrate de instalar los requerimientos: pip install -r requirements.txt")
    sys.exit(1)
...
- Para las magnitudes, rescata los números exactos reportutilities (ej. "aumenta 4.5%", "coeficiente 0.12").
...
- **Contexto Geográfico/Temporal:** [País, ciudad, años analizutilities]

## 4. Datos y Fuentes
- [Breve punteo de los datos usutilities, niveles de agregación]

## 5. Resultutilities y Magnitudes Principales (Números Duros)
...
def main():
    parser = argparse.ArgumentParser(description="Extractor de notas de literatura usando Gemini 2.5 Flash.")
    parser.add_argument("file", nargs="?", help="Ruta a un archivo PDF específico (opcional). Si no se provee, procesa toda la carpeta 'papers/'.")
    parser.add_argument("--api-key", help="Tu API Key de Google (opcional, por defecto busca en variables de entorno GOOGLE_API_KEY).", default=None)
    args = parser.parse_args()

    api_key = args.api_key or os.environ.get("GOOGLE_API_KEY")
    if not api_key:
        print("❌ ERROR CRÍTICO: No se encontró la variable de entorno GOOGLE_API_KEY ni se proveyó vía --api-key.")
    ...
    if not pdf_files:
        ...
        print(f"⚠️ Por favor, deposita tus PDFs descargutilities allí e intenta de nuevo.")
        sys.exit(0)
        
    print(f"🔍 Encontrutilities {len(pdf_files)} PDFs en cola...")
